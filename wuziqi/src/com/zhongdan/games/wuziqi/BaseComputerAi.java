...
public class BaseComputerAi extends BasePlayer {
	...
	private void initRange(Vector comuters, Vector humans) {
		currentRange.xStart = ((Point) humans.elementAt(0)).getX() - RANGE_STEP;
		currentRange.yStart = ((Point) humans.elementAt(0)).getY() - RANGE_STEP;
		currentRange.xStop = ((Point) humans.elementAt(0)).getX() + RANGE_STEP;
		currentRange.yStop = ((Point) humans.elementAt(0)).getY() + RANGE_STEP;
		for (int i = 0; i < humans.size(); i++) {
			Point point = (Point) humans.elementAt(i);
		...
		for (int i = 0; i < comuters.size(); i++) {
			Point point = (Point) comuters.elementAt(i);
			if (point.getX() - RANGE_STEP < currentRange.xStart) {
				currentRange.xStart = point.getX() - RANGE_STEP;
			} else if (point.getX() + RANGE_STEP > currentRange.xStop) {
				currentRange.xStop = point.getX() + RANGE_STEP;
	...
	private Point doAnalysis(Vector comuters, Vector humans) {
	...
	private Point getFirstPoint(Vector humans) {
		Point point = (Point) humans.elementAt(0);
	...
	// 开始分析，扫描所有空白点，形成第一次分析结果
	private Point doFirstAnalysis(Vector comuters, Vector humans) {
		...
		for (int i = 0; i < size; i++) {
			computerPoint = (Point) allFreePoints.elementAt(i);
	...
	private Point doComputerSencondAnalysis(Hashtable firstResults, Vector sencodResults) {
		Vector list = null;
		SencondAnalysisResult sr = null;
		Enumeration keys = firstResults.keys();
		for (; keys.hasMoreElements();) {
			Point p = (Point) keys.nextElement();
			sr = new SencondAnalysisResult(p);
			list = (Vector) firstResults.get(p);
			for (int i = 0; i < list.size(); i++) {
				FirstAnalysisResult result = (FirstAnalysisResult) list.elementAt(i);
				if (result.count == 4) {
					if (result.aliveState == ALIVE) {// 经过前面的过滤，双方都排除了绝杀棋，有活4就下这一步了，再下一步就赢了
						return result.point;// 如果有绝杀，第一轮已返回，在此轮活4已经是好的棋子，直接返回，不再往下分析
					} else {
						sr.halfAlive4++;
						computer4HalfAlives.addElement(sr);
					}
				} else if (result.count == 3) {
					if (result.aliveState == ALIVE) {
						sr.alive3++;
						if (sr.alive3 == 1) {
							computer3Alives.addElement(sr);
						} else {
							computerDouble3Alives.addElement(sr);
						}
					} else {
						sr.halfAlive3++;
						computer3HalfAlives.addElement(sr);
					}
				} else {// 半活2在第一阶段已被排除，不再处理
					sr.alive2++;
					if (sr.alive2 == 1) {
						computer2Alives.addElement(sr);
					} else {
						computerDouble2Alives.addElement(sr);
					}
				}
			}
			sencodResults.addElement(sr);
		}
		// 没有找到活4
		return null;
	}
	...
	private Point doHumanSencondAnalysis(Hashtable firstResults, Vector sencodResults) {
		Vector list = null;
		SencondAnalysisResult sr = null;
		Enumeration keys = firstResults.keys();
		for (; keys.hasMoreElements();) {
			...
			for (int i = 0; i < list.size(); i++) {
				...
				if (result.count == 4) {
					if (result.aliveState == ALIVE) {
						human4Alives.addElement(sr);
					} else {
						sr.halfAlive4++;
						human4HalfAlives.addElement(sr);
					}
				} else if (result.count == 3) {
					if (result.aliveState == ALIVE) {
						sr.alive3++;
						if (sr.alive3 == 1) {
							human3Alives.addElement(sr);
						} else {
							humanDouble3Alives.addElement(sr);
						}
					} else {
						sr.halfAlive3++;
						human3HalfAlives.addElement(sr);
					}
				} else {
					sr.alive2++;
					if (sr.alive2 == 1) {
						human2Alives.addElement(sr);
					} else {
						humanDouble2Alives.addElement(sr);
					}
				}
			}
			sencodResults.addElement(sr);
	...
	private Point doThirdAnalysis() {
		if (!computer4HalfAlives.isEmpty()) {
			return ((SencondAnalysisResult) computer4HalfAlives.elementAt(0)).point;
		}
		System.gc();
		sleep(300);
		compareVector(computerSencodResults);
		...
		compareVector(humanSencodResults);
		...
		// 拿出各自排第一的，谁好就下谁
		return ((SencondAnalysisResult) computerSencodResults.elementAt(0)).point;
	}
	...
	protected Point getBestPoint(Vector myBest, Vector yourSencodResults) {
		if (!myBest.isEmpty()) {
			if (myBest.size() > 1) {
				for (int i = 0; i < yourSencodResults.size(); i++) {
					SencondAnalysisResult your = (SencondAnalysisResult) yourSencodResults.elementAt(i);
					if (myBest.contains(your)) {
						return your.point;
					}
				}
				return ((SencondAnalysisResult) myBest.elementAt(0)).point;
			} else {
				return ((SencondAnalysisResult) myBest.elementAt(0)).point;
			}
		}
	...
	private final Hashtable computerFirstResults = new Hashtable();
	private final Hashtable humanFirstResults = new Hashtable();
	// 第二次总结果
	protected final Vector computerSencodResults = new Vector();
	protected final Vector humanSencodResults = new Vector();
	// 第二次分结果，电脑
	protected final Vector computer4HalfAlives = new Vector(2);
	protected final Vector computerDouble3Alives = new Vector(4);
	protected final Vector computer3Alives = new Vector(5);
	protected final Vector computerDouble2Alives = new Vector();
	protected final Vector computer2Alives = new Vector();
	protected final Vector computer3HalfAlives = new Vector();

	// 第二次分结果，人类
	protected final Vector human4Alives = new Vector(2);
	protected final Vector human4HalfAlives = new Vector(5);
	protected final Vector humanDouble3Alives = new Vector(2);
	protected final Vector human3Alives = new Vector(10);
	protected final Vector humanDouble2Alives = new Vector(3);
	protected final Vector human2Alives = new Vector();
	protected final Vector human3HalfAlives = new Vector();
	...
	private void initAnalysisResults() {
		...
		humanFirstResults.clear();
		// 第二次总结果
		computerSencodResults.removeAllElements();
		humanSencodResults.removeAllElements();
		// 第二次分结果
		computer4HalfAlives.removeAllElements();
		computerDouble3Alives.removeAllElements();
		computer3Alives.removeAllElements();
		computerDouble2Alives.removeAllElements();
		computer2Alives.removeAllElements();
		computer3HalfAlives.removeAllElements();

		// 第二次分结果，人类
		human4Alives.removeAllElements();
		human4HalfAlives.removeAllElements();
		humanDouble3Alives.removeAllElements();
		human3Alives.removeAllElements();
		humanDouble2Alives.removeAllElements();
		human2Alives.removeAllElements();
		human3HalfAlives.removeAllElements();
		System.gc();
	}

	// 加入到第一次分析结果中
	private void addToFirstAnalysisResult(FirstAnalysisResult result, Hashtable dest) {
		if (dest.containsKey(result.point)) {
			((Vector) dest.get(result.point)).addElement(result);
		} else {
	...
	// 第二次分析结果类
	class SencondAnalysisResult implements Comparable {
		int alive4 = 0;
		// 活3数量
		int alive3 = 0;
		// 半活4，一头封的
	...
	private int compareVector(Vector v) {
		for (int i = 0; i < v.size() - 1; i++) {
			for (int j = i + 1; j < v.size(); j++) {
				SencondAnalysisResult oneResult = (SencondAnalysisResult) v.elementAt(i);
				SencondAnalysisResult another = (SencondAnalysisResult) v.elementAt(j);
				if (oneResult.alive4 > another.alive4) {
					// return -1;
				}
				if (oneResult.alive4 < another.alive4) {
					// return 1;
					swap(oneResult, another);
				}
	...
	private void swap(SencondAnalysisResult a, SencondAnalysisResult b) {
		SencondAnalysisResult temp = null;
		temp = a;
		a = b;
		b = temp;
	}
	...
	private int compareTowResult(SencondAnalysisResult oneResult, Object another) {
		if (oneResult.alive4 > ((SencondAnalysisResult) another).alive4) {
			return -1;
		}
		if (oneResult.alive4 < ((SencondAnalysisResult) another).alive4) {
			return 1;
		}
		if (oneResult.halfAlive4 > ((SencondAnalysisResult) another).halfAlive4) {
			return -1;
		}
		if (oneResult.halfAlive4 < ((SencondAnalysisResult) another).halfAlive4) {
			return 1;
		}
		if (oneResult.alive3 > ((SencondAnalysisResult) another).alive3) {
			return -1;
		}
		if (oneResult.alive3 < ((SencondAnalysisResult) another).alive3) {
			return 1;
		}
		if (oneResult.alive2 > ((SencondAnalysisResult) another).alive2) {
			return -1;
		}
		if (oneResult.alive2 < ((SencondAnalysisResult) another).alive2) {
			return 1;
		}
		if (oneResult.halfAlive3 > ((SencondAnalysisResult) another).halfAlive3) {
			return -1;
		}
		if (oneResult.halfAlive3 > ((SencondAnalysisResult) another).halfAlive3) {
			return 1;
	...
	private FirstAnalysisResult tryAndCountResult(Vector myPoints, Vector enemyPoints, Point point, int direction) {
		int x = point.getX();
		int y = point.getY();
	...
	private void countPoint(Vector myPoints, Vector enemyPoints, Point point, FirstAnalysisResult fr, int direction, boolean forward) {
	...
	// 在某个方向上是否还能下到满五个棋子
	private int maxCountOnThisDirection(Point point, Vector enemyPoints, int direction, int count) {
