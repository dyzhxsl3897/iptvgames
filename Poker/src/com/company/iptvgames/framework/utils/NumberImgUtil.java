...
public class NumberImgUtil {
	...
	public static Vector updateNumber(int number, Image numberImg, int x, int y, int pos) {
		...
		if (number > 0) {
			while (numberSplit > 0) {
				Sprite s = new Sprite(numberImg, width / 5, height / 2);
		...
		if (pos == (Graphics.LEFT | Graphics.TOP)) {
			for (int i = digitNo - 1; i >= 0; i--) {
				...
				int posX = x + width / 5 * (digitNo - i - 1);
				int posY = y;
