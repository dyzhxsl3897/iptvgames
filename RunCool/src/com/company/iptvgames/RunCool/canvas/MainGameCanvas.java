...
public class MainGameCanvas extends GameCanvas implements Runnable {
	...
	private void loadImages() {
		ImageRes.getInstance().loadImage("bgImg0", ImageUtil.createImage("/map/b0.png"));
	...
	private void createNew(String type) {
		...
		if(type.equalsIgnoreCase("BG")){
			...
			if(num > 1){				
				...
				((ObstacleObject)ObstacleVector.elementAt(num - 1)).setPosition(tempX + rand + previousWidth, 
						tempY + GameConst.Player.WALKHEIGHT - ((ObstacleObject)ObstacleVector.elementAt(num - 1)).getHeight());
			}else{
				((ObstacleObject)ObstacleVector.elementAt(0)).setPosition(rand,
						tempY + GameConst.Player.WALKHEIGHT - ((ObstacleObject)ObstacleVector.elementAt(0)).getHeight());
			}
		}
	}
