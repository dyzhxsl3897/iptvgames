			...
			while (isRunning) {
				...
				for (int i = 0; i < zdSprites.length; i++) {
					...
					if (zdSprites[i].getWhose() == 0) {// 我的子弹与地方碰撞
						for (int h = 0; h < tankSprites.length; h++) {
							if (tankSprites[h].isVisible() && tankSprites[h].getCx() >= 20 && zdSprites[i].collidesWith(tankSprites[h], false)) {
								zdSprites[i].bz();
								if (count > once) {
						...
						if (tankSprite.isVisible() && !tankSprite.getBhSprite().isVisible() && zdSprites[i].collidesWith(tankSprite, false)) {
							tankSprite.fh();
							myCount--;
						}
				...
				for (int i = 0; i < sprite.length; i++) {
					for (int j = 0; j < sprite[i].length; j++) {
						...
						// 敌方坦克与障碍物碰撞
						for (int h = 0; h < tankSprites.length; h++) {
							if (tankSprites[h].isVisible() && (tankSprites[h].collidesWith(sprite[i][j], false) || bj(tankSprites[h]))) {
				...
				for (int i = 0; i < tankSprites.length; i++) {
					if (tankSprites[i].isVisible()) {
						if (tankSprites[i].isVisible() && rdm.nextInt(15) == 1 && tankSprites[i].getCx() >= 20)
							fire(tankSprites[i]);
						if (step == 4) {
							if (rdm.nextInt(10) == 0) {
